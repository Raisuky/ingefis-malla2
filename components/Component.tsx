...
// Declaración de las constantes de localStorage
const LOCAL_STORAGE_KEY = 'approvedSubjects'; // Llave para asignaturas aprobadas
const LOCAL_STORAGE_KEY_IN_PROGRESS = 'inProgressSubjects'; // Llave para asignaturas en curso
const LOCAL_STORAGE_GRADES_KEY = 'grades'; // Llave para calificaciones
...
const Curriculum: React.FC = () => {
  ...
  const [grades, setGrades] = useState<{ [code: string]: number }>({}); // Mapa de calificaciones por asignatura
  ...
  // Cargar asignaturas aprobadas, en curso y calificaciones desde localStorage al iniciar
  useEffect(() => {
  ...
  // Guardar las asignaturas aprobadas, en curso y calificaciones en localStorage cada vez que cambien
  useEffect(() => {
  ...
  // Manejar la selección de asignaturas aprobadas y permitir ingresar una calificación
  const handleApproveSubject = (code: string) => {
    ...
    if (parsedGrade >= 1.0 && parsedGrade <= 7.0) {
      setGrades({ ...grades, [code]: parsedGrade });
  ...
  // Calcular el promedio ponderado acumulado
  ...
  const createNode = (subject: Subject) => {
    ...
    return (
      <div
        ...
        onDoubleClick={() => handleApproveSubject(subject.code)}
        onContextMenu={(e) => {
          e.preventDefault();
      ...
      >
        ...
        <p className="text-sm">Créditos: {subject.credits}</p>
        {isApproved && <p className="text-sm">Calificación: {grades[subject.code]}</p>}
      </div>
    );
  };
  ...
  return (
    <div className={`container mx-auto px-4 py-8 ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-black'}`}>
      ...
      {/* Mostrar el promedio ponderado acumulado */}
      <div className="mb-8">
        <h2 className="text-lg font-bold">Promedio Ponderado Acumulado: {calculateWeightedAverage()}</h2>
      </div>
