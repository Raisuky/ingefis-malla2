...
const Curriculum: React.FC = () => {
  ...
  const handleApproveSubject = (code: string) => {
    if (approvedSubjects.includes(code)) {
      ...
      setInProgressSubjects(inProgressSubjects.filter(subjectCode => subjectCode !== code)); // Remover de "en curso"
    }
  };
  ...
  const filteredSubjects = subjects.filter(subject => {
    const matchesSearch = subject.name.toLowerCase().includes(searchQuery.toLowerCase()) || subject.code.includes(searchQuery);
